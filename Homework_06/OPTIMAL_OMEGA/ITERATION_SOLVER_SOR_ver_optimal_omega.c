...
int main(void)
{
	...
	sprintf(filename, "optimal_omega_OpenMP_threads=%d_lattice_size=%d.txt",N_threads,N);
	output_optimal = fopen(filename,"w");
	fprintf(output_optimal, "#Omega\tIterations\tError\tPreparation_Time(s)\tCalculation_Time(s)\n");
...
#	pragma omp parallel for
	...
	double omega_interval = 0.001;

	for (double omega=1.99; omega<2.0-omega_interval; omega+=omega_interval)
	{
		double start = omp_get_wtime();	
		field_even = calloc((N*N)/2,sizeof(double));
		field_odd = calloc((N*N)/2,sizeof(double));
		...
		printf("Starts computation with omega = %.4f , error = %.8e ...\n", omega, error);
		iter = 0;
...
#		pragma omp parallel for 
		...
		fprintf(output_optimal, "%.4f\t%ld\t%.8e\t%.4f\t%.4f\n", omega, iter, error, preparation_time, computation_time);
	}
