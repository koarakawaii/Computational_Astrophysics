...
int main(int argc, char* argv[])
{
	...
	DDOT(shift, &norm_rank, rho_rank, rho_rank);
...
void DAXPY_X(int shift, double c, double *A, double *B)
{
	for (int idx=0; idx<shift; idx++)
		A[idx] = c*A[idx] + B[idx];
}

void DAXPY_Y(int shift, double c, double *A, double *B)
{
	for (int idx=0; idx<shift; idx++)
		B[idx] = c*A[idx] + B[idx];
}

void DDOT(int shift, double* dot, double* A, double* B)
{
	*dot = 0.0;
	for (int idx=0; idx<shift; idx++)
		*dot += A[idx]*B[idx];
