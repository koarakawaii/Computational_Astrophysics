...
int main(void)
{
	...
	long iter, iter_max;
    int *dimension_level;
	double *rho, *p, *A_p, *field_analytic, *error_block, *r_temp;
	...
	while (sqrt(error)/norm>criteria&&iter<iter_max)
	{
		if (N_level!=0)
		{
			...
			for (int l=N_level; l>0; l--)
			{
				...
				norm_in = error_in;
				cudaMemcpy(p, r_level[l], dl*dl*sizeof(double), cudaMemcpyDeviceToDevice);
...
//				while (sqrt(error_in)/norm_in>0.99)
				{
			        ...
			        cublasDdot(handle, dl*dl, r_level[l], 1, r_level[l], 1, &temp);
			        beta = temp/error_in;
			        DAXPY<<<bpg,tpb>>>(dl, beta, p, r_level[l]);
			        error_in = temp;
//					printf("%.6e\n", error_in);
				}
			}
