...
//    A_inverse = (double*)calloc(row*row,sizeof(double));
	...
	{
		RESTRICT_2D<<<bpg,tpb>>>(dimension_level[1], r_level[0], r_level[1]);
		cublasDdot(handle, dimension_level[1]*dimension_level[1], r_level[1], 1, r_level[1], 1, &error);
		printf("%.8e\n",error);
		cublasDgemv(handle, CUBLAS_OP_N, row, row, &one, A_inverse, row, r_level[1], 1, &zero, field_level[1], 1);

//		cudaDeviceSynchronize();
//		output_field = fopen("simulated_field_distribution_GPU_TGCG_proto.txt","w");
//		FPRINTF(output_field, dimension_level[1], 1., field_level[1]);
//		return EXIT_SUCCESS;
