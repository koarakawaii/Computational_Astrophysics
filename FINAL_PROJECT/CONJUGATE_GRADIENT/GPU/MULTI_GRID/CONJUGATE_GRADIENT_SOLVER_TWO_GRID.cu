...
//		printf("%.8e\n", error_in);
		norm_in = error_in;
		cudaMemcpy(p, r_level[1], dl*dl*sizeof(double), cudaMemcpyDeviceToDevice);
		while (sqrt(error_in)/norm_in>0.95)
		{
			LAPLACIAN<<<bpg,tpb>>>(dl, dx, photon_mass, factor/4., p, A_p);
	        cublasDdot(handle, dl*dl, p, 1, A_p, 1, &temp);
	        alpha = error_in/temp;
	        temp = -alpha;
	        cublasDaxpy(handle, dl*dl, &temp, A_p, 1, r_level[1], 1);
	        cublasDaxpy(handle, dl*dl, &alpha, p, 1, field_level[1], 1);
	        cublasDdot(handle, dl*dl, r_level[1], 1, r_level[1], 1, &temp);
	        beta = temp/error_in;
	        DAXPY<<<bpg,tpb>>>(dl, beta, p, r_level[1]);
	        error_in = temp;
//			printf("%.6e\n", error_in);
		}
